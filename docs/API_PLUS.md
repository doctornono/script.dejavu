...
Paginated channel / universe listing. Query: `page`, `pageSize` (max 100), optional `type` (`UNIVERSE` | `EDITORIAL` | `PLATFORM` | `THEMATIC` | `CREATOR` | `AWARDS` | `FRANCHISE`), optional `scope` (`mine` | `following`; omit = explore).
...
Channel detail plus attached lists (Kodi-shaped) and a short item preview (`movieId` / `tvShowId`). Full section rows: existing `GET /lists/{id}/items` / RPC `get_list_items`.

Kodi fallback on 404: same `not_found` envelope.
