...
`dv.get_capabilities()` (sync, no HTTP) returns `{ protocol, addonVersion, features }`. Feature-detect `episode_status`, `media_status_cache`, `show_progress`, `resolve_batch`, `sync_cursor`, `plugin_widgets`, `channels`.
...
channels = dv.get_channels(channel_type="UNIVERSE", page=1, page_size=20)
channel = dv.get_channel(channel_id)
# Full section items: dv.get_list_items(list_id) using list.id from channel["data"]["lists"]
...
| `get_channels` | `type` (`UNIVERSE` / `EDITORIAL` / …), `scope` (`mine` / `following`), `page`, `page_size` |
| `get_channel` | `channel_id` (alias `id`) |
| `get_dashboard` | — |
