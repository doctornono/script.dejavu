...
# This module can be invoked by another add-on (notably a skin) as well as by
# Kodi directly. Resolve the add-on explicitly so settings and localized
# strings never inherit the caller's context.
ADDON = xbmcaddon.Addon("script.dejavu")
...
def _format_localized(string_id, *args):
    """Format a localized message without letting a bad translation abort an action."""
    template = _ls(string_id) or ""
    try:
        return template % args if len(args) != 1 else template % args[0]
    except (TypeError, ValueError):
        xbmc.log(
            "[dejaVu] Invalid localized format for string %s: %r" % (string_id, template),
            xbmc.LOGWARNING,
        )
        return template or " ".join(str(arg) for arg in args)
...
def add_to_list_dialog(api=None, info=None):
    ...
    if already:
        ...
        if result is None:
            ...
            return
        xbmcgui.Dialog().notification(
            "dejaVu", _format_localized(30104, list_name), xbmcgui.NOTIFICATION_INFO, 3000
        )
    ...
    if result is None:
        ...
        return
    xbmcgui.Dialog().notification(
        "dejaVu", _format_localized(30103, list_name), xbmcgui.NOTIFICATION_INFO, 3000
    )
