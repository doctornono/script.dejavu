...
Supported actions (method format: 'script.dejavu.ACTION'):
  READ
    ...
    get_channels          params: page, page_size, type, scope
    get_channel           params: channel_id (alias id)
...
class DejaVuMonitor(xbmc.Monitor):
    ...
    def _handle_get_channels(self, params):
        return self.api.get_channels(
            page=params.get("page", 1),
            page_size=params.get("page_size", 20),
    ...
    def _handle_get_channel(self, params):
        return self.api.get_channel(
            params.get("channel_id") or params.get("id"),
        )
