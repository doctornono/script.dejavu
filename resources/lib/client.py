...
class DejaVuClient:
    ...
    def call(self, action, params=None):
        ...
        try:
            sender = xbmcaddon.Addon("script.dejavu").getAddonInfo("id") or "script.dejavu"
        except Exception:
    ...
    def get_channels(self, channel_type=None, scope=None, page=1, page_size=20):
        params = {"page": page, "page_size": page_size}
        if channel_type:
            params["type"] = channel_type
        if scope:
            params["scope"] = scope
        return self.call("get_channels", params)

    def get_channel(self, channel_id):
        return self.call("get_channel", {"channel_id": channel_id})
